...
def check_json_precision():
    """Make sure json library being used does not lose precision converting DOGECOIN values"""
...
def initialize_chain(test_dir, num_nodes, cachedir):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            logger.debug("initialize_chain: dogecoind started, waiting for RPC to come up")
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr=None):
    ...
    dogecoind_processes[i] = subprocess.Popen(args, stderr=stderr)
    logger.debug("initialize_chain: dogecoind started, waiting for RPC to come up")
...
def assert_start_raises_init_error(i, dirname, extra_args=None, expected_msg=None):
    with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
        try:
            ...
            assert 'dogecoind exited' in str(e) #node must have shutdown
...
def stop_node(node, i):
    ...
    return_code = dogecoind_processes[i].wait(timeout=BITCOIND_PROC_WAIT_TIMEOUT)
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s DOGECOIN too low! (Should be %s DOGECOIN)"%(str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s DOGECOIN too high! (Should be %s DOGECOIN)"%(str(fee), str(target_fee)))
