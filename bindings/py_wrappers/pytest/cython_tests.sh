...
# build:
python3-dbg bindings/py_wrappers/libdogecoin/setup.py build_ext --build-lib `pwd`/bindings/py_wrappers/pytest/ --build-temp `pwd`/ --force --user
...
python3-dbg -Wd -X tracemalloc=5 bindings/py_wrappers/pytest/transaction_test.py 
deactivate
rm -rf .venv
